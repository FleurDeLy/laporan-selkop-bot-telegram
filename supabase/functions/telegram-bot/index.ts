...
async function sendDocument(
  ...
  formData.append(
    "document",
    new Blob([documentBuffer], { type: "text/csv" }),
    filename,
  );
...
async function handleRecapCommand(chatId: number, text: string): Promise<void> {
  try {
    ...
    if (
      args.length !== 3 || !dateRegex.test(args[1]) || !dateRegex.test(args[2])
    ) {
      await sendMessage(
        chatId,
        "⚠️ *Format Salah*\nGunakan: `/recap DD-MM-YYYY DD-MM-YYYY`\nContoh: `/recap 01-06-2026 08-06-2026`",
        undefined,
        "Markdown",
      );
      return;
    ...
    if (!recapData || recapData.length === 0) {
      await sendMessage(
        chatId,
        `📭 Tidak ada data untuk periode *${startInput}* s/d *${endInput}*.`,
        undefined,
        "Markdown",
      );
      return;
    ...
    const stats: Record<
      string,
      { total: number; done: number; failed: number; pending: number }
    > = {};
    let grandTotal = 0;
    let grandDone = 0;
    let grandFailed = 0;
    ...
    recapData.forEach((task: any) => {
      ...
      if (!stats[branch]) {
        stats[branch] = { total: 0, done: 0, failed: 0, pending: 0 };
      }
    ...
    let reportText =
      `📋 <b>Rekap Tugas — ${startInput} s/d ${endInput}</b>\n\n`;
    ...
    reportText +=
      `<i>Ketik /recap DD-MM-YYYY DD-MM-YYYY untuk rekap rentang tanggal.</i>`;
...
async function notifyPicsForCompletedTask(
  supabaseClient: any,
  staffChatId: number,
  taskName: string,
  photoUrl: string,
  ...
  try {
    ...
    for (const pic of pics) {
      await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/sendPhoto`, {
        ...
        console.error(
          `Failed to send report to PIC ${pic.telegram_chat_id}:`,
          err,
        )
      );
    }
  ...
  try {
    ...
    if (update.callback_query) {
      ...
      if (chatId && data && data.startsWith("task_")) {
        ...
        if (error) {
          ...
          await sendMessage(
            chatId,
            `📸 Siap, <b>${firstName}</b>! Silakan kirim foto sebagai bukti untuk menyelesaikan tugas ini.`,
          );
        }
      }
      ...
      if (chatId && data && data.startsWith("assign_")) {
        ...
        const { data: targetUser } = await supabase.from("Users").select("name")
          .eq("telegram_chat_id", targetId).maybeSingle();
        const targetName = targetUser?.name || "Staff";

        await supabase.from("Users").update({ draft_assignee_id: targetId }).eq(
          "telegram_chat_id",
          chatId,
        );
        await sendMessage(
          chatId,
          `📝 Baik, silakan ketik deskripsi tugas untuk <b>${targetName}</b>:`,
        );
      }
    ...
    if (update.message && update.message.photo) {
      ...
      const { data: user } = await supabase.from("Users").select(
        "active_task_id, name, outlet",
      ).eq("telegram_chat_id", chatId).maybeSingle();

      if (!user || !user.active_task_id) {
        await sendMessage(
          chatId,
          "⚠️ Kamu belum memilih tugas! Ketik /tasks dan pilih tugas terlebih dahulu.",
        );
        return new Response("OK", { status: 200 });
      }
      ...
      try {
        const { data: taskData } = await supabase.from("Tasks").select(
          "task_name, assigned_by, created_at",
        ).eq("id", activeTaskId).maybeSingle();
        ...
        const { data: publicUrlData } = supabase.storage.from("task-photos")
          .getPublicUrl(fileName);
        const photoUrl = publicUrlData.publicUrl;

        const completedAt = new Date();
        const { data: updatedTask, error: updateError } = await supabase.from(
          "Tasks",
        ).update({
          status: "done",
          photo_url: photoUrl,
          completed_at: completedAt.toISOString(),
        }).eq("id", activeTaskId).select("task_name").single();
        if (updateError) throw updateError;
        await supabase.from("Users").update({ active_task_id: null }).eq(
          "telegram_chat_id",
          chatId,
        );

        // Build WITA timestamps directly — no view query needed
        const completedAtWita = completedAt.toLocaleString("sv-SE", {
          timeZone: "Asia/Makassar",
        });
        const assignedAtWita = taskData?.created_at
          ? new Date(taskData.created_at).toLocaleString("sv-SE", {
            timeZone: "Asia/Makassar",
          })
          : "-";
        ...
        await sendMessage(
          chatId,
          "❌ Gagal menyimpan foto. Silakan coba kirim ulang.",
        );
      }
      return new Response("OK", { status: 200 });
    }
    ...
    if (update.message && update.message.text) {
      ...
      if (!text.startsWith("/")) {
        const { data: draftUser } = await supabase.from("Users").select(
          "draft_assignee_id",
        ).eq("telegram_chat_id", chatId).maybeSingle();
        ...
        if (draftUser && draftUser.draft_assignee_id) {
          ...
          const { data: assigneeUser } = await supabase.from("Users").select(
            "outlet",
          ).eq("telegram_chat_id", assigneeId).maybeSingle();
          await supabase.from("Tasks").insert({
            telegram_chat_id: assigneeId,
            task_name: taskDescription,
            status: "pending",
            assigned_by: chatId,
            outlet: assigneeUser?.outlet || null,
          });
          await supabase.from("Users").update({ draft_assignee_id: null }).eq(
            "telegram_chat_id",
            chatId,
          );
      ...
      if (text === "/start") {
        ...
        const { data: existingUser } = await supabase.from("Users").select(
          "name, role",
        ).eq("telegram_chat_id", chatId).maybeSingle();
        ...
        if (!existingUser) {
          ...
          await sendMessage(
            chatId,
            `✅ Halo <b>${existingUser.name}</b>, kamu sudah terdaftar di sistem!\n\nKetik /help untuk panduan.`,
          );
        }
      } else if (text === "/stop") {
        ...
        if (error) {
          ...
          await sendMessage(
            chatId,
            `🚫 Kamu telah berhenti berlangganan. Semua tugasmu telah dihapus dari sistem. Ketik /start jika ingin kembali.`,
          );
        }
      } else if (text === "/tasks") {
        const { data: tasks, error } = await supabase.from("Tasks").select(
          "id, task_name",
        ).eq("telegram_chat_id", chatId).eq("status", "pending");
        if (error || !tasks || tasks.length === 0) {
          await sendMessage(
            chatId,
            "🎉 Tidak ada tugas pending untukmu saat ini!",
          );
          return new Response("OK", { status: 200 });
        }
        const buttons = tasks.map(
          (task) => [{
            text: `✅ Selesaikan: ${task.task_name}`,
            callback_data: `task_${task.id}`,
          }]
        );
        await sendMessage(
          chatId,
          "📋 <b>Tugas Kamu Hari Ini:</b>\nSilakan pilih tugas yang ingin diselesaikan:",
          { inline_keyboard: buttons },
        );
      } else if (text === "/help") {
        const helpMsg = `🤖 <b>Panduan Bot Laporan Harian</b>\n\n` +
        ...
        await sendMessage(
          ...
          `📊 <b>Status Bot</b>\n\nStatus kamu: ${
            ...
            isRegistered
              ? "Ketik /tasks untuk melihat tugasmu."
              : "Ketik /start untuk mendaftar."
          }`,
        );
      } else if (text === "/addtask") {
        const { data: sender } = await supabase.from("Users").select("role").eq(
          "telegram_chat_id",
          chatId,
        ).maybeSingle();
        if (sender?.role !== "pic") {
          await sendMessage(
            chatId,
            "❌ Akses ditolak. Perintah ini hanya untuk PIC.",
          );
          return new Response("OK", { status: 200 });
        }
        const { data: staffList } = await supabase.from("Users").select(
          "telegram_chat_id, name",
        ).eq("role", "staff");
        if (!staffList || staffList.length === 0) {
          await sendMessage(
            chatId,
            "❌ Tidak ada staff yang terdaftar saat ini.",
          );
          return new Response("OK", { status: 200 });
        }
        const buttons = staffList.map(
          (staff) => [{
            text: `👤 ${staff.name}`,
            callback_data: `assign_${staff.telegram_chat_id}`,
          }]
        );
        await sendMessage(
          chatId,
          "📋 <b>Pilih staff yang akan diberi tugas:</b>",
          { inline_keyboard: buttons },
        );
        ...
        try {
          ...
          let monitorMsg =
            `📊 *LIVE MONITORING DASHBOARD (HARI INI)*\n*Scope:* ${displayScope}\n\n`;

          if (!staffList || staffList.length === 0) {
            monitorMsg += "Tidak ada staff yang terdaftar di outlet ini.";
          } else {
            for (const staff of staffList) {
              const rawStaffTasks = (todayTasks || []).filter((t: any) =>
                t.telegram_chat_id === staff.telegram_chat_id
              );
              ...
              for (const task of rawStaffTasks) {
                ...
                if (
                  !existingTask || task.status === "Selesai" ||
              ...
              const completedTasks = staffTasks.filter((t: any) =>
                t.status === "Selesai" || t.status === "DONE" ||
                t.status === "done"
              ).length;
              const currentActivity = staff.active_task_id
                ? "🟢 (Sedang Bekerja)"
                : "⚪ (Standby)";

              monitorMsg += `👷‍♂️ *${staff.name}* (${staff.outlet})\n`;
              monitorMsg +=
                `Status: ${completedTasks}/${totalTasks} Selesai ${currentActivity}\n`;

              if (totalTasks > 0) {
                for (const task of staffTasks) {
                  const icon =
                    (task.status === "Selesai" || task.status === "DONE" ||
                        task.status === "done")
                      ? "✅"
                      : "⌛";
